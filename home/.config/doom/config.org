...
(map!
 ...
 (:desc "file" :prefix "f"
   ...
   :desc "find" :n  "r" #'helm-org-rifle-config)
...
;; TODO
;; 1. Split upto these headers, then reverse find (--last) to the first available header
;;    When no header is found, just insert the header at the top of the file
;; 2. Find the next comment header, if none is available go to the end of the file
;; 3. Insert your import statement
...
(defun +MM|add-import-to-file (file)
  ...
  (let* ((is-local (s-contains? "./" file))
    ...
    (pcase (--first (save-excursion (search-forward it nil t)) '("EXTERNALS" "LOCALS" "MAIN")))
