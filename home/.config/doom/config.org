...
(cond
 ((s-contains? "Florians-MBP" system-name)
...
(map! :leader (:desc "Notes" :prefix "n"
                ...
                :desc "Reading List"         :n "r" #'+org-reading-list/org-open-reading-list-file
...
#+END_SRC
**** ACTIVE Counsel Org Effort
#+BEGIN_SRC elisp :tangle no
(defun counsel-org-effort ()
  (ivy-read "Set effort: " org-duration-units :action #'org-set-effort))
#+END_SRC
...
(defun +org-agenda ()
  ...
  (let ((org-agenda-files
         (--map (concat org-directory "/" it) '("inbox.org"
                                                "home.org"
                                                "Work/work.org")))
        (org-agenda-ndays 5)
        ...
        (org-super-agenda-groups '((:name "Schedule"
                                          :time-grid t
                                          :date today
                                          :discard (:scheduled past :deadline past))
                                   (:name "Unscheduled" :todo "TODO NEXT"))))
    (org-agenda-list)))
#+END_SRC
...
**** (e) Eshell
:PROPERTIES:
:SOURCE:   [[https://github.com/xandeer/.doom/blob/master/modules/private/xandeer/%252Beshell.el][.doom/+eshell.el at master · xandeer/.doom]]
:END:
...
(map! :leader
      :desc "eshell" "e" nil
      (:prefix ("e" . "eshell")
        :desc "Create a new eshell window below the current one."             "j"   #'+eshell/split-below
        :desc "Create a new eshell window to the right of the current one."   "l"   #'+eshell/split-right
...
(defun my-evil-org-agenda-set-keys ()
  (evil-define-key 'motion org-agenda-mode-map
    ...
    "dSp" (λ! (org-agenda-schedule t "+1d 7pm"))
    "dSw" (λ! (org-agenda-schedule t "+1d 8am"))
    "dSS" (λ! (org-agenda-schedule t "+1d"))
